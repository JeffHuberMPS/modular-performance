        ...
        {showForm && (
          <div style={styles.formCard}>
            <div style={styles.formGrid}>
              ...
              <Field label="Date">
                {/* max = today. The field had no upper bound, so a mis-keyed year (2099 instead of
                    2026) saved happily and then poisoned the log. You cannot log a night that has
                    not happened yet, so the picker should not offer one. */}
                <input type="date" value={form.date} max={todayStr()}
                  onChange={(e) => setForm({ ...form, date: e.target.value > todayStr() ? todayStr() : e.target.value })}
                  style={styles.input} />
              </Field>
...
const SleepTargetCard = ({ rows }) => {
  ...
  const accent = __GRAY ? "#C9A020" : "#9b6bc9";
  const rgba   = __GRAY ? "201,160,32" : "155,107,201";
  /* Hands the bedtime to the phone's calendar. A closed web app cannot wake itself at a set time
     (Notification Triggers never shipped, iOS gives a PWA no background scheduling, and real push
     needs a paid scheduler), so the OS alarm does the job instead. Free, and more reliable. */
  const addReminder = () => {
    try {
      const ics = RecoverySleep.icsReminder(t.bedMinutes, t.targetLabel, new Date());
      if (!ics) return;
      const url = URL.createObjectURL(new Blob([ics], { type: "text/calendar;charset=utf-8" }));
      const a = document.createElement("a");
      a.href = url; a.download = "mps-bedtime.ics"; a.rel = "noopener";
      document.body.appendChild(a); a.click();
      setTimeout(() => { try { a.remove(); URL.revokeObjectURL(url); } catch (err) {} }, 2000);
    } catch (err) {}
  };
      ...
      {t.bedLabel && (
        <div style={{ marginTop: 12 }}>
          <button type="button" onClick={addReminder}
            style={{ display: "inline-flex", alignItems: "center", gap: 7, cursor: "pointer",
                     background: "transparent", border: `1px solid rgba(${rgba},0.55)`,
                     color: accent, fontWeight: 700, fontSize: 12.5, lineHeight: 1.2,
                     padding: "9px 15px", borderRadius: 999,
                     fontFamily: "'Inter', system-ui, -apple-system, sans-serif" }}>
          ...
          <div style={{ fontSize: 11, color: "#8a8a8a", marginTop: 7, lineHeight: 1.5 }}>
            Adds a repeating reminder to your phone&rsquo;s own calendar, so it still works with the app closed.
          </div>
        </div>
      )}
    </section>
