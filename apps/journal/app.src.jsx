    ...
    // Live "listening" indicator. gAnalyser holds the active session's audio analyser (one mic at a
    // time); MicWave reads it every frame and draws sound bars that react to your real voice level.
    const gAnalyser = { current: null };
      ...
      useEffect(() => {
        ...
        const draw = () => {
          ...
          for (let i=0;i<NB;i++) {
            ...
            const amp = Math.min(1, Math.max(0.16, level*3.4) * wobble);
    ...
    function WriteView({ entries, onSave, tdOverride }) {
      ...
      const streamRef = useRef(null);   // live mic MediaStream — keeps the button lit + feeds the sound bars
      const actxRef   = useRef(null);   // AudioContext for the analyser
      const rafRef    = useRef(0);      // silence-watch animation frame
      const listenRef = useRef(false);  // is a listening session active

      // Tear down the whole listening session (mic, speech, analyser, timers) and drop the highlight.
      const endSession = () => {
        ...
        try { if (actxRef.current) actxRef.current.close(); } catch(e){}
        actxRef.current = null;
        gAnalyser.current = null;
        setRecKey(null);
      };
      useEffect(() => () => { endSession(); }, []);
      useEffect(() => { if (!recErr) return; const t = setTimeout(() => setRecErr(''), 3600); return () => clearTimeout(t); }, [recErr]);

      const startVoice = async (fieldId, getBase, setText) => {
        if (!SpeechRec) { setRecErr('Voice typing needs Chrome (desktop or Android).'); return; }
        endSession();   // stop any mic already running
        // 1) Open a real mic stream. This is the reliable part: it keeps the button lit and drives the
        //    live sound bars, even if Chrome's speech-to-text is being finicky.
        let stream;
        try { stream = await navigator.mediaDevices.getUserMedia({ audio:true }); }
        catch(e){ const n = e && e.name;
          if (n==='NotAllowedError' || n==='SecurityError') setRecErr('Microphone is blocked. Click the lock icon left of the address bar, allow the mic, then try again.');
          else if (n==='NotFoundError' || n==='OverconstrainedError') setRecErr('No microphone found on this device.');
          else setRecErr('Could not open the microphone. Check your mic, then try again.');
          return; }
        streamRef.current = stream;
        listenRef.current = true;
        setRecKey(fieldId);            // highlight ON now — stays lit until endSession()

        // 2) Analyser: real-time level for the sound bars + silence auto-off.
        let analyser = null;
        try {
          const AC = window.AudioContext || window.webkitAudioContext;
          const actx = new AC(); actxRef.current = actx;
          try { actx.resume(); } catch(e){}
          analyser = actx.createAnalyser(); analyser.fftSize = 256; analyser.smoothingTimeConstant = 0.6;
          actx.createMediaStreamSource(stream).connect(analyser);
          gAnalyser.current = analyser;   // MicWave reads this to draw the bars
        } catch(e){ /* bars/silence-detect are optional; keep going */ }

        // 3) Auto-off: when you stop talking for ~4s, shut the mic off by itself. If we DID hear real
        //    audio but speech-to-text never returned a word, say so — that pins the fault on Chrome's
        //    voice engine (not the mic), instead of failing silently.
        let sawAudio = false, sawResult = false;
        if (analyser) {
          const SILENCE_MS = 4000, VOICE_LEVEL = 0.045;
          ...
          const watch = () => {
            ...
            let s = 0; for (let i=0;i<data.length;i++) s += data[i];
            if (s/data.length/255 > VOICE_LEVEL) { lastVoice = Date.now(); sawAudio = true; }
            if (Date.now() - lastVoice > SILENCE_MS) {
              if (sawAudio && !sawResult) setRecErr('Your mic works — I can see the sound — but Chrome did not turn it into words. Reload the page and try once more.');
              endSession(); return;
            }
          ...
          setTimeout(() => { if (listenRef.current) endSession(); }, 20000);   // no analyser: hard cap
        }

        // 4) Speech-to-text. Runs alongside the stream; if Chrome ends it early while we're still
        //    listening, quietly restart it so dictation keeps flowing and the button stays lit.
        ...
        const startSR = () => {
          let recog;
          try { recog = new SpeechRec(); } catch(e){ return; }
          recog.continuous = true; recog.interimResults = true; recog.lang = 'en-US';
          recog.onresult = (e) => {
            restarts = 0;   // producing text means it's healthy — allow unlimited restarts
            sawResult = true;
            let interim = '';
          ...
          recog.onerror = (e) => {
            const err = e && e.error;
            if (err === 'not-allowed' || err === 'service-not-allowed') { setRecErr('Microphone is blocked. Allow mic access in your browser, then try again.'); endSession(); }
          ...
          recog.onend = () => {
            ...
            recRef.current = null;
            if (listenRef.current && restarts < 60) { restarts++; setTimeout(() => { if (listenRef.current) startSR(); }, 150); }
          };
