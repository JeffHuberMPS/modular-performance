/* ══════════════════════════════════════════════════════════
   ...
   Modules:
     ...
     RecoveryData          schema, validation, CRUD, edit history
     ...
     RecoverySync          additive-union merge, tombstones, offline queue
     RecoveryLegacyBridge  old Rest & Recovery fields -> new schema

   VERIFIED: 356/356 assertions pass in the sandbox harness at
   Desktop\Jarvis\MPS_Builds\mps_recovery_sandbox.html

   NOT YET VERIFIED: RecoverySync.FirestoreAdapter has never run
   against real Firestore. Merge logic is fully tested against an
   in-memory double only. Test on the preview before trusting it.
...
/* ============================================================
   MPS RECOVERY — WEAPON 1: RECOVERY DATA STRUCTURE
   Locked per handoff v3.0 Part 15. Storage, validation, and the
   recalc pipeline. All math is delegated to Weapon 3.
   ============================================================ */

const RecoveryData = (function(){

  const SANDBOX = RecoveryEngineConfig.namespace.indexOf('sandbox') > -1;
  const KEY = uid => `${__nsPrefix()}_${uid}`;
  ...
  function validate(input){
    ...
    if (!input.bedtime || !timeRe.test(input.bedtime)) errors.push('bedtime');
    if (!input.wakeTime || !timeRe.test(input.wakeTime)) errors.push('wakeTime');
    SLIDERS.forEach(k=>{
      const v = input[k];
      if (!Number.isInteger(v) || v < 1 || v > 10) errors.push(k);
    });
    if (!input.date || !/^\d{4}-\d{2}-\d{2}$/.test(input.date)) errors.push('date');
    return {valid: errors.length === 0, errors};
  }

  /* ---------- the record (PART 15 schema) ---------- */
  function buildRecord(input, history, existing){
    const v = validate(input);
    if (!v.valid) return {ok:false, errors:v.errors};

    const now = new Date().toISOString();
    const s = S.score(input, history);   // Weapon 3 does all the math.
    ...
    const rec = {
      ...
      physicalRecovery: input.physicalRecovery,
      calmness: input.calmness,

      sleepDuration: s.sleepDuration,
      sleepDurationScore: s.sleepDurationScore,
      sliderAverage: s.sliderAverage,
      recoveryScore: s.recoveryScore,

      recoveryGrade: s.recoveryGrade,
      recoveryStatus: s.recoveryStatus,
      levelName: s.levelName,
      dotCount: s.dotCount,
      dotColor: s.dotColor,

      pushMeter: s.pushMeter,
      pushMessage: s.pushMessage,

      lowestMetric: s.lowestMetric,
      selectedAction: existing ? (existing.selectedAction ?? null) : null,
      actionCompleted: existing ? !!existing.actionCompleted : false,

      generatedInsights: existing ? (existing.generatedInsights || []) : [],
      generatedCoachingId: existing ? (existing.generatedCoachingId ?? null) : null,

      editHistory: existing ? (existing.editHistory || []).slice() : []
    };

    if (existing){
      const changed = Object.keys(input).filter(k =>
        k in existing && existing[k] !== input[k]);
      if (changed.length) rec.editHistory.push({editedAt:now, changedFields:changed});
    }
  ...
  function get(userId, date){ return loadAll(userId)[date] || null; }
  function list(userId){
    return Object.values(loadAll(userId)).sort((a,b)=> a.date < b.date ? -1 : 1);
  }
  // Save = validate, recalc every dependent field, persist, queue cloud sync.
  function save(input){
    const history  = list(input.userId);
    const existing = get(input.userId, input.date);
    const built = buildRecord(input, history, existing);
    if (!built.ok) return built;

    const all = loadAll(input.userId);
    all[input.date] = built.record;
    localStorage.setItem(KEY(input.userId), JSON.stringify(all));
    queueSync(built.record);
    return built;
  }
  // Weapon 5 writes only the action fields. No recalculation, no edit
  // history entry, because the inputs did not change.
  function updateAction(userId, date, patch){
    const all = loadAll(userId);
    const rec = all[date];
    if (!rec) return null;
    if ('selectedAction'  in patch) rec.selectedAction  = patch.selectedAction;
    if ('actionCompleted' in patch) rec.actionCompleted = !!patch.actionCompleted;
  ...
  function remove(userId, date){
    const all = loadAll(userId);
    delete all[date];
    localStorage.setItem(KEY(userId), JSON.stringify(all));
  }
  function clear(userId){ localStorage.removeItem(KEY(userId)); }

  // Cloud sync stub. Wired to Firestore additive-union sync when this
  // leaves the sandbox. Sandbox never writes to the cloud.
  // Weapon 11 owns the transport. This just hands off the write.
  // Guarded so the data layer still works if sync is not loaded yet.
  function queueSync(record){
    try {
      if (typeof RecoverySync === 'undefined') return;
      RecoverySync.enqueue(record.userId, {
        type:'set', date:record.date, record:record
      });
    } catch(e){ /* never let a sync failure block a local save */ }
  }

  // Used by a pull to install a merged set in one write.
  function replaceAll(userId, rows){
    const all = {};
    (rows || []).forEach(r => { all[r.date] = r; });
    localStorage.setItem(KEY(userId), JSON.stringify(all));
    return Object.keys(all).length;
  }

  // Scoring is re-exported so callers have one door to knock on.
  return {SLIDERS, TIERS:S.TIERS, validate, buildRecord,
          save, get, list, remove, clear, updateAction, replaceAll,
          sleepMinutes:S.sleepMinutes, durationScore:S.durationScore,
          sliderAverage:S.sliderAverage, grade:S.grade, tier:S.tier,
          pushMeter:S.pushMeter, lowestMetric:S.lowestMetric, score:S.score};
})();
...
/* ============================================================
   MPS RECOVERY — WEAPON 11: CLOUD SYNC
   Locked per handoff v3.0 Part 15.
   Additive union: a record missing from the other side is never
   deleted, it is added. Deletes only happen through an explicit
   tombstone, so one device can never silently wipe another.
   The adapter is pluggable. Firestore when firebase is present,
   an in-memory double otherwise, which is what the sandbox and
   the tests use. The sandbox NEVER touches live collections.
   ============================================================ */

const RecoverySync = (function(){

  // Namespace comes from RecoveryEngineConfig, never hardcoded.
  // Live merge target is users/{uid}/recovery. The sandbox is namespaced
  // so it is impossible to write into the real pillar's data.
  const COLLECTION = RecoveryEngineConfig.namespace;
  const SANDBOX    = COLLECTION.indexOf('sandbox') > -1;
  const QKEY  = uid => `${__nsPrefix()}_queue_${uid}`;
  const TSKEY = uid => `${__nsPrefix()}_syncts_${uid}`;
  const TOMB  = uid => `${__nsPrefix()}_tomb_${uid}`;

  /* ---------- adapters ---------- */
  // In-memory double. Deterministic, offline-simulating, test-friendly.
  function MemoryAdapter(){
    ...
    return {
      ...
      get(id){
        ...
        return Promise.resolve(docs[id] ? JSON.parse(JSON.stringify(docs[id])) : null);
      },
      set(id, data){
        if (!online) return Promise.reject(new Error('offline'));
        docs[id] = JSON.parse(JSON.stringify(data));
      ...
      list(){
        if (!online) return Promise.reject(new Error('offline'));
        return Promise.resolve(Object.values(docs).map(d => JSON.parse(JSON.stringify(d))));
      }
    };
  }

  // Firestore, using the same compat SDK and path shape as the live pillar.
  function FirestoreAdapter(uid){
    ...
    return {
      ...
      get: id => db.doc(`${path}/${id}`).get()
        .then(s => s.exists ? s.data() : null),
      set: (id, data) => db.doc(`${path}/${id}`).set(data).then(() => true),
      remove: id => db.doc(`${path}/${id}`).delete().then(() => true),
      list: () => db.collection(path).get()
  ...
  function use(a, uid){ adapter = a; if (uid) currentUser = uid; }
  function getAdapter(){ return adapter; }

  // Wires to real Firestore only when firebase is loaded AND a user is
  // signed in. Otherwise the in-memory double keeps everything local.
  function attach(){
    try {
      if (typeof firebase === 'undefined' || !firebase.auth) return false;
      const u = firebase.auth().currentUser;
      if (!u) return false;
      adapter = FirestoreAdapter(u.uid);
      currentUser = u.uid;
      return true;
  ...
  /* ---------- tombstones ---------- */
  // An explicit delete is recorded so it can propagate. Without this,
  // additive union would resurrect anything deleted on another device.
  function loadTombstones(uid){
    try { return JSON.parse(localStorage.getItem(TOMB(uid))) || {}; }
    catch(e){ return {}; }
  }
  function tombstone(uid, date){
    const t = loadTombstones(uid);
    t[date] = new Date().toISOString();
    localStorage.setItem(TOMB(uid), JSON.stringify(t));
  }
  function clearTombstones(uid){ localStorage.removeItem(TOMB(uid)); }

  /* ---------- offline queue ---------- */
  function loadQueue(uid){
    try { return JSON.parse(localStorage.getItem(QKEY(uid))) || []; }
    catch(e){ return []; }
  }
  function saveQueue(uid, q){
    localStorage.setItem(QKEY(uid), JSON.stringify(q));
  }
  // Newest write per date wins in the queue. No point pushing five
  // versions of the same day.
  function enqueue(uid, op){
    const q = loadQueue(uid).filter(x => x.date !== op.date);
    q.push(op);
    saveQueue(uid, q);
    return q;
  }
  function queueLength(uid){ return loadQueue(uid).length; }
  function clearQueue(uid){ localStorage.removeItem(QKEY(uid)); }

  /* ---------- merge (the important part) ---------- */
  // Additive union. Local and remote both survive. A record present on
  // one side only is kept, never dropped. When both sides have the same
  // date, the newer updatedAt wins. Tombstoned dates are removed.
  function merge(localRows, remoteRows, tombstones){
    const byDate = {};
    (localRows  || []).forEach(r => { byDate[r.date] = r; });
    (remoteRows || []).forEach(r => {
      const mine = byDate[r.date];
      if (!mine){ byDate[r.date] = r; return; }
      const a = Date.parse(mine.updatedAt || 0) || 0;
      const b = Date.parse(r.updatedAt   || 0) || 0;
      if (b > a) byDate[r.date] = r;
    });
    Object.keys(tombstones || {}).forEach(date => {
      const rec = byDate[date];
      if (!rec) { delete byDate[date]; return; }
      // A write newer than the delete resurrects the day on purpose.
      const del = Date.parse(tombstones[date]) || 0;
      const upd = Date.parse(rec.updatedAt || 0) || 0;
      if (upd <= del) delete byDate[date];
    });
  ...
  function push(uid){
    ...
    if (!q.length) return Promise.resolve({pushed:0, queued:0});
    if (!adapter.isOnline()) return Promise.resolve({pushed:0, queued:q.length});

    let pushed = 0;
    const remaining = [];
    return q.reduce((chain, op) => chain.then(() => {
      const work = op.type === 'delete'
        ? adapter.remove(op.date)
        : adapter.set(op.date, op.record);
      return work.then(() => { pushed++; })
  ...
  function pull(uid){
    uid = uid || currentUser;
    if (!adapter.isOnline()) return Promise.resolve({merged:0, offline:true});
    return adapter.list().then(remote => {
      const local  = RecoveryData.list(uid);
      const merged = merge(local, remote, loadTombstones(uid));
      RecoveryData.replaceAll(uid, merged);
      localStorage.setItem(TSKEY(uid), new Date().toISOString());
      return {merged: merged.length, offline:false};
    }).catch(() => ({merged:0, offline:true}));
  }

  // Push what is waiting, then pull and merge. Safe to call repeatedly.
  function sync(uid){
    uid = uid || currentUser;
    return push(uid).then(p => pull(uid).then(q => ({
      pushed:p.pushed, queued:p.queued,
      merged:q.merged, offline:q.offline
    })));
  }

  function lastSynced(uid){
    return localStorage.getItem(TSKEY(uid || currentUser));
  }
  ...
  function status(uid){
    ...
    return {
      ...
      pending: queueLength(uid),
      lastSynced: lastSynced(uid),
  ...
  function reset(uid){
    uid = uid || currentUser;
    clearQueue(uid); clearTombstones(uid);
    localStorage.removeItem(TSKEY(uid));
  }

  return {use, getAdapter, attach, MemoryAdapter, FirestoreAdapter,
          merge, push, pull, sync, enqueue, loadQueue, queueLength,
          clearQueue, tombstone, loadTombstones, clearTombstones,
          status, lastSynced, reset, COLLECTION};
})();


const RecoveryLegacyBridge = (function(){

  // Old Rest & Recovery fields -> new locked schema.
  // Soreness and Restlessness are inverted: high soreness means low
  // physical recovery, high restlessness means low sleep quality.
  const MAPPING = [
    {from:'sleepTime',    to:'bedtime',          how:'direct'},
    {from:'wakeTime',     to:'wakeTime',         how:'direct'},
    {from:'energy',       to:'energy',           how:'direct'},
    {from:'clarity',      to:'mentalClarity',    how:'direct'},
    {from:'soreness',     to:'physicalRecovery', how:'inverted'},
    {from:'restlessness', to:'sleepQuality',     how:'inverted'}
  ];

  // Fields with no home in the new schema, and the one with no source.
  const DROPPED = ['recovery','hours','restingHR','hrv','weight'];
  const UNMAPPED = ['calmness'];

  const invert = v => (v == null ? null : 11 - v);

  function convert(oldEntry){
    const out = {date: oldEntry.date};
    const missing = [];
    MAPPING.forEach(m => {
      const v = oldEntry[m.from];
      if (v == null || v === ''){ missing.push(m.to); return; }
      out[m.to] = m.how === 'inverted' ? invert(v) : v;
    });
    UNMAPPED.forEach(k => missing.push(k));
    return {record: out, missing, complete: missing.length === 0};
  }

  // A dry run. Reports what would happen without writing anything.
  function preview(oldEntries){
    const rows = (oldEntries || []).map(convert);
    const fieldCounts = {};
    rows.forEach(r => r.missing.forEach(f =>
      fieldCounts[f] = (fieldCounts[f] || 0) + 1));
    return {
      total: rows.length,
      complete: rows.filter(r => r.complete).length,
      incomplete: rows.filter(r => !r.complete).length,
      missingByField: fieldCounts,
      dropped: DROPPED.slice(),
      rows
    };
  }

  return {MAPPING, DROPPED, UNMAPPED, convert, preview};
})();
...
window.RecoveryData         = RecoveryData;
...
window.RecoveryLegacyBridge = RecoveryLegacyBridge;
